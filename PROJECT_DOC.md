...
4. **사실 검증** — `FACT_VERIFICATION.md` 7단계 절차 적용 (§10에서 게이트로 강제)
5. **체크리스트 통과** — §10 발행 전 최종 체크리스트
6. **배포** — Naver Maps API 도메인 등록 (§8)
...
- **🔒 필수 동반 문서**: [`FACT_VERIFICATION.md`](./FACT_VERIFICATION.md) — 모든 신규 지역 브리핑은 이 검증 방법론을 거쳐야 발행 가능. 적용 시점은 §7.1(사전), §10(발행 전 최종 게이트).
...
**콘텐츠의 가로 폭은 `.wrap`(최외곽 컨테이너) 한 곳에서만 결정한다.** 개별 요소(`<p>`, `h1.title`, `.standfirst`, `.sec-deck`, footnote 등)에 별도의 `max-width`를 걸지 않는다.

| 분류 | max-width 사용 | 이유 |
|---|---|---|
| `.wrap` (최외곽 컨테이너) | ✅ 1280px | 모든 콘텐츠의 기준 너비 |
| 미디어 쿼리 `@media (max-width: ...)` | ✅ | 반응형 브레이크포인트 |
| 작은 UI 배지 / 장식선 (`.legend`, `.kicker::after`) | ✅ | 본문 콘텐츠가 아닌 부속 요소 |
| **본문 텍스트 요소** (`<p>`, `h1.title`, `.standfirst`, `.sec-deck`) | ❌ | `.wrap`이 이미 결정함. 추가 제약은 통일감 깨짐 |
| 인라인 `style="max-width:Npx"` | ❌ | 한 곳만 좁아져서 시각적 불균형 야기 |

**왜?** 타이포그래피 가독성을 이유로 단락 너비를 좁히면, 부모(.wrap)가 더 넓은 공간을 가질 때 한 단락만 좁아 보이는 시각적 불균형이 생긴다. 줄당 글자수 제약이 필요하면 `.wrap` 자체의 max-width를 조정하라(현재 1280px).
...
- [ ] 헤드라인·standfirst 초안
- [ ] 🔒 **`FACT_VERIFICATION.md` §8 "검증 전 ☑️" 항목 통과** — 검증 가능한 클레임 목록화 + 우선순위 매기기 완료
...
> **🔒 절대 게이트**: 아래 카테고리 중 "사실 검증" 게이트를 통과하지 못하면 발행 불가. 데이터·디자인이 완벽해도 사실 오류·편향이 있는 자료는 시리즈 신뢰도를 깬다.

### 🔒 사실 검증 (FACT_VERIFICATION.md 적용)
> 자세한 절차는 [`FACT_VERIFICATION.md`](./FACT_VERIFICATION.md). 아래는 발행 전 통과해야 하는 최소 게이트.

- [ ] **클레임 추출·우선순위 완료** (FV §1~§2) — HIGH 우선 클레임 목록 확보
- [ ] **HIGH 클레임 모두 2개 이상 독립 출처로 교차 검증** (FV §4.1)
- [ ] **시점 검증 통과** (FV §4.2) — 모든 수치에 기준 시점 명시 + 자료 작성 시점과의 갭 확인
- [ ] **오류 패턴 7종 점검** (FV §5) — 내부 비일관성 / 시점 혼동 / 오래된 데이터 / 검증 불가 수치 / 환산 오류 / 출처 불명확 / 함의 차이
- [ ] **편향 5종 점검** (FV §6) — 누락 / 프레이밍 / 출처 / 시점 선택 / 비교군
- [ ] **🔴 명백한 오류 0건 확인**
- [ ] **🟡 검증 어려움·🟠 중요 누락 항목은 본문 또는 footer에 명시적 한계 표기** (FV §7.4 외부 발행본 디스클로저 원칙 — 🔴 정정 이력은 표기 안 함)
- [ ] **자기 검증 1회** (FV §9.4) — 본인이 사용한 출처를 다시 한번 점검
